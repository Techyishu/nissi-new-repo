        ...
        <main className="min-h-screen bg-white overflow-hidden mt-[120px]" ref={containerRef}>
            ...
            <section className="py-24 bg-white container mx-auto px-6">
                <div className="text-center mb-20">
                    ...
                    <h2 className="text-4xl md:text-5xl font-serif font-bold text-slate-900 mb-4">Comprehensive Curriculum</h2>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 auto-rows-[320px]">
                    {subjects.map((subject, index) => (
                        <motion.div
                            ...
                            className={`relative group cursor-pointer rounded-3xl overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 ${index === 0 || index === 3 ? 'md:col-span-2' : ''}`}
                        >
                            <Image
                                src={subject.image}
                                alt={subject.title}
                            ...
                            />
                            <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-black/20 group-hover:from-black/90 transition-colors" />

                            <div className="absolute inset-0 p-8 flex flex-col justify-end">
                                <div className={`${subject.color} w-14 h-14 rounded-xl flex items-center justify-center text-white mb-4 shadow-2xl transform group-hover:-translate-y-2 group-hover:scale-110 transition-all duration-300`}>
                                    <subject.icon size={28} />
                                </div>
                                <h3 className="text-3xl md:text-4xl font-bold text-white mb-3">{subject.title}</h3>
                                <p className="text-white/90 line-clamp-2 group-hover:text-white transition-colors text-lg">
                                    {subject.description}
                                </p>
                            </div>
                        </motion.div>
                ...
                <AnimatePresence>
                    {selectedId && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
                            ...
                            <motion.div
                                layoutId={selectedId}
                                className="relative w-full max-w-4xl bg-white rounded-3xl overflow-hidden shadow-2xl z-10 border border-gray-100"
                            >
